...
var (
	_ cache.Store = (*Store[string])(nil)
)
...
func (s *Store[_]) DeregisterCache(name string) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store[_]) SetLogger(log slog.Logger) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Store[_]) withLogger(level slog.LogLevel) (slog.Logger, bool) {
	if s.log != nil {
...
func (s *Store[_]) withDebug() (slog.Logger, bool) {
